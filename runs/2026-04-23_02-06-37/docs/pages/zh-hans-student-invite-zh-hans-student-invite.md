...
- 可执行操作：back、multilingual-list、button、ion-button、加入、返回
- 可录入字段：text / input
- 页面加载时触发网络请求 3 个
...
- 页面信号：SReading-最智能的免费中文阅读辅助工具；最终 URL: https://sr.sigmareading.com/zh-hans/student-invite；标题: （无标题）；交互元素: 8 个；工作流验证: 班级邀请详情工作流=PASS/PASS
- 交互信号：button: back；button: multilingual-list；button: button；button: ion-button；button: button；button: 加入；button: 返回；field: text
- 工作流信号：班级邀请详情工作流: 识别 4 条详情文本信号、7 个控件信号。
...
| 班级邀请详情工作流 | verify | 验证 /zh-hans/student-invite 是否已展示详情内容，并暴露当前页可继续操作的控件。 | 至少识别 2 条可用于描述详情页的文本信号；识别当前详情页是否存在按钮、输入框或其它后续操作控件 | 识别 4 条详情文本信号、7 个控件信号。<br>读取详情内容: 识别 4 条详情文本：SReading；请输入班级码。；加入 | PASS |
...
| 阶段 | 结果 | 摘要 | 关键细节 |
|------|------|------|----------|
| 发现功能 | PASS | 发现 4 个候选功能点。 | 页面上下文识别: SReading-最智能的免费中文阅读辅助工具<br>操作按钮发现: back<br>输入字段发现: text |
| 验证功能 | PASS | 执行 1 个页面工作流验证。 | 班级邀请详情工作流: 识别 4 条详情文本信号、7 个控件信号。 |
| 修改探索脚本 | PASS | 本轮未发现必须调整的页面探索脚本。 | （无） |
| 再次验证功能 | PASS | 当前页的已配置工作流在本轮内完成了再次验证。 | 班级邀请详情工作流 / 读取详情内容: PASS<br>班级邀请详情工作流 / 检查详情操作控件: PASS |
| 发现新功能 | PASS | 再次发现 10 条新选项/字段线索。 | 班级邀请详情工作流: back<br>班级邀请详情工作流: multilingual-list<br>班级邀请详情工作流: button |
...
- 识别到的选项：back、multilingual-list、button、ion-button、text、加入、返回
...
| 检查详情操作控件 | /zh-hans/student-invite | 识别当前详情页是否存在按钮、输入框或其它后续操作控件 | 识别 7 个控件：back；multilingual-list；button；ion-button | PASS | （无） |
